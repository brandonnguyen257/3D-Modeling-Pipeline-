...
def getObjectMask(imageList,objectMaskThresh):
    """
    Given an image where an object is placed and another image where the object is removed,
    return a mask for left and right cams that determines which pixel belongs to the foreground/object
    Parameters
    ----------
    imageList: list of colored numpy images
        0: imgL
        1: imgL_bg
        2: imgR
        3: imgR_bg
    
    objectMaskThresh : float
       Threshold to determine if a pixel is foreground or background
       
    Returns
    -------
    tmaskL : 2D numpy.array (dtype=bool)
        Array mask for the camL that indicates pixels where the object is located
        
    tmaskR : 2D numpy.array (dtype=bool)
         Array mask for the camR that indicates pixels where the object is located
    
    """    
    #convert imgae to grayscale
    temp_ImgL=convertColorToGray(imageList[0])
    temp_ImgL_bg=convertColorToGray(imageList[1])
    temp_ImgR=convertColorToGray(imageList[2])
    temp_ImgR_bg=convertColorToGray(imageList[3])
    
    #calculate left and right mask
    maskL=(temp_ImgL-temp_ImgL_bg)
    tmaskL=np.where(maskL<objectMaskThresh,0.0,1.0)
    maskR=(temp_ImgR-temp_ImgR_bg)
    tmaskR=np.where(maskR<objectMaskThresh,0.0,1.0)
    return tmaskL, tmaskR

def convertColorToGray(img):
    """
    convert img into grayscale
    
    Parameters
    ---------
    img: colored numpy image array
    
    returns
    -----
    tempImg: grayscale numpy image
    """
    
    tempImg=img
    if (tempImg.dtype == np.uint8):
        tempImg = tempImg.astype(float) 
    if(tempImg.ndim==3):
        tempImg=np.mean(tempImg,axis=2)     
    return tempImg
...
def reconstruct(imprefixL,imprefixR,threshold,camL,camR,imageList,objectThreshold):
    """
    ...
    """
    objMaskL,objMaskR=getObjectMask(imageList,objectThreshold)
    ...
    #itearte through all points
    for i in range(N):
        #get pixel color from left image
        xL=pts2L[0][i]
        yL=pts2L[1][i]
        leftColor=np.array([imageL[yL][xL]])
        
        #get pixel color from right image
        xR=pts2R[0][i]
        yR=pts2R[1][i]
        rightColor=np.array([imageR[yR][xR]])
        
        #set pixel color to be average of left and right
        avgColor=(rightColor+leftColor)/2.0
        avgColor=avgColor.T

        color=np.append(color,avgColor,axis=1)
